...
> The property template key is not configured. Run `the setup-library skill` with your Figma template library link first.
...
- The property template key is stored in the template configuration under `templateKeys.propertyOverview` and is configured via `the setup-library skill`. This is a dedicated property template with the header already set to "Property" — no renaming needed.
