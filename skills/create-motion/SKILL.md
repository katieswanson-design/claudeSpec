...
- `the create-motion skill` + pasted JSON (from clipboard, exported via `motion/export-timeline.jsx`)
...
> The motion spec template key is not configured. Run `the setup-library skill` with your Figma template library link first.
...
- The motion spec template key is stored in the template configuration under `templateKeys.motionSpec` and is configured via `the setup-library skill`.
