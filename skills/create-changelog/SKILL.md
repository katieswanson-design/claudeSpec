...
Read the template configuration (from conversation context, Claude memory, or user-provided keys) and extract the `changelog` value from the `templateKeys` object.

Save this key as `CHANGELOG_TEMPLATE_KEY`. If the key is empty, tell the user:
> The changelog template key is not configured. Run `the setup-library skill` with your Figma template library link first.
...
- The changelog template key is stored in the template configuration under `templateKeys.changelog` and is configured via `the setup-library skill`.
