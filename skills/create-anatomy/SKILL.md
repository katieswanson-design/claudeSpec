...
- [ ] Step 2: Read template key from configuration (memory or context)
...
> The anatomy template key is not configured. Run `the setup-library skill` with your Figma template library link first.
...
Always load fonts before modifying any text. Figma silently drops text changes if fonts aren't loaded. Use this pattern at the start of every `figma_execute` call that modifies text:
...
In claudeSpec, template keys and font family come from conversation context or Claude's memory — not from a local `uspecs.config.json` file. Ask the user for template keys if they're not in memory, or run the setup-library skill first.
