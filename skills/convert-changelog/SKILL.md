...
Extract content from an existing Figma changelog and convert it to structured JSON that can be used with claudeSpec.
...
Use `figma_execute` to check if the changelog uses the claudeSpec template structure:

```javascript
const node = await figma.getNodeByIdAsync('NODE_ID');
if (!node) return { error: 'Node not found' };

// Check for claudeSpec template structure
...
#### If Structured (claudeSpec template)
