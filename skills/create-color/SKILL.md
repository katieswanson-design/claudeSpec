...
> The color annotation template key is not configured. Run `the setup-library skill` with your Figma template library link first.
...
- The color annotation template key is stored in the template configuration under `templateKeys.colorAnnotation` and is configured via `the setup-library skill`.
...
These notes document fixes and adaptations discovered during claudeSpec testing with Claude:

### Preview Frame Placement
When placing component instances into `#preview` frames, always inspect the child structure first. Preview frames contain nested placeholder children (e.g., "Light theme preview placeholder" and "Dark theme preview placeholder"). Append instances into these specific child frames — not into `#preview` directly. Center instances by setting `primaryAxisAlignItems` and `counterAxisAlignItems` to `'CENTER'` on the placeholder.

### Table Title Placeholders
The `#color-table-template` contains a `#table-title` frame with `{Table title}` placeholder text. After cloning variant sections, always fill this text with the variant name (e.g., "Primary — Color tokens").

### Token Extraction Optimization
For components with many variants, skip axes that don't affect color (typically Size and Icon Only) by passing a `SKIP_AXES` parameter. This reduces extraction time dramatically.
