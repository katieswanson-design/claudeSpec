...
- [ ] Step 3: Read template key from configuration (memory or context)
...
Read the template configuration (from conversation context, Claude memory, or user-provided keys) and extract:
...
> The screen reader template key is not configured. Run `the setup-library skill` with your Figma template library link first.
...
- The screen reader template key is stored in the template configuration under `templateKeys.screenReader` and is configured via `the setup-library skill`.
...
## claudeSpec-Specific Notes

### Template Structure
The Screen Reader template has a nested structure: State → Platform → Table → Rows. Each state section contains `#section` children for each platform (iOS, Android, Web), and each `#section` contains a `#state-table` with header and data rows.

### Announcement Example Field
The `#header-row` in each platform table has an `#announcement` frame containing `{announcementExample}` placeholder text. This is DYNAMIC content — the actual screen reader announcement string for that state (e.g., "Button CTA, Button"). It must be filled by the script, not left as a placeholder.

### Platform Section Cloning
When creating multiple platforms within a state, clone from the first `#section` child — don't re-clone the entire state template. After cloning, fill the `#platform-title` text in each section.

### Row Value Cells
Data rows use `#prop-name`, `#prop-value`, and `#prop-notes`. The `#prop-value` cell contains `{value}` placeholder text that must be filled with platform-specific accessibility property values.
