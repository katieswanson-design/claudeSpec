...
> The structure template key is not configured. Run `the setup-library skill` with your Figma template library link first.
...
- The structure template key is stored in the template configuration under `templateKeys.structureSpec` and is configured via `the setup-library skill`.
